...
class ResearcherAgent:
    def __init__(
        ...
        self.deep_research = deep_research or DeepResearchService()
    ...
    def analyze(self, ticker: str, article_urls: list[str] | None = None, context: str | None = None) -> ResearchFinding:
        ...
        logger.info("Researcher completed for %s", symbol)
        return finding
    ...
    def _generate_research_finding(self, ticker: str, articles: list[NewsArticle], context: str | None) -> ResearchFinding:
        ...
        messages = [
            ChatMessage(
                role="system",
                content=(
                    "You are the Researcher Agent. Return only strict JSON matching: "
                    "ticker, sentiment, sentiment_score, summary, catalysts, concerns, articles. "
                    "Do not calculate financial metrics. Use narrative and sentiment analysis only."
                ),
            ),
            ChatMessage(
                role="user",
